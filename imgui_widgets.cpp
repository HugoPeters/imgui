...
ImRect ImGui::GetWindowScrollbarRect(ImGuiWindow* window, ImGuiAxis axis)
{
    ...
    float scrollbar_size = window->ScrollbarSizes[axis ^ 1]; // (ScrollbarSizes.x = width of Y scrollbar; ScrollbarSizes.y = height of X scrollbar)

    if (scrollbar_size == 0)
    {
        scrollbar_size = GetStyle().ScrollbarSize;
    }
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImRect total_bb(pos, pos + ImVec2(square_sz + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f), label_size.y + style.FramePaddingCheckbox.y * 2.0f));
    ItemSize(total_bb, style.FramePaddingCheckbox.y);
    ...
    if (label_size.x > 0.0f)
        RenderText(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + style.FramePaddingCheckbox.y), label);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        {
            bool wk_no_enter = (flags & ImGuiInputTextFlags_WkNoEnterConfirm) != 0;
...
bool ImGui::TreeNodeEx(const char* label, ImGuiTreeNodeFlags flags, const ImVec2& frameSizeOffset)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;

    return TreeNodeBehavior(window->GetID(label), flags, label, NULL, frameSizeOffset);
}
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end, const ImVec2& frameSizeOffset)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
