...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end, const ImVec2& frameSizeOffset)
{
    ...
    const ImRect interact_bb = (display_frame || (flags & ImGuiTreeNodeFlags_SpanAllAvailWidth) != 0) ? frame_bb : ImRect(frame_bb.Min.x, frame_bb.Min.y, frame_bb.Min.x + text_width + style.ItemSpacing.x*2, frame_bb.Max.y);
